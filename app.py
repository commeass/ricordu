...
def stream_cmd(cmd, on_line, env=None):
    ...
    p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                         text=True, bufsize=1, env=env or ENV, cwd=BASE, start_new_session=True)
    JOB["proc"] = p
    try:
...
def run_pipeline(opts):
    try:
        ...
        rc = stream_cmd([PY, "diaporama.py", "ai_select", sb], on_ai, env=env_ai)
...
def render_stage(sb_path):
    ...
    h = archive_montage(montage, S)                   # succès garanti ici -> une entrée d'historique par rendu
